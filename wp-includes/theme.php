...
function get_stylesheet_uri() {
	...
	$stylesheet_uri = $stylesheet_dir_uri . '/dist/css/app.css';
	/**
		...
		if ( is_array( $cached_roots ) ) {
			foreach ( $cached_roots as $theme_dir => $theme_root ) {
				...
				$found_themes[ $theme_dir ] = array(
					'theme_file' => $theme_dir . '/dist/css/app.css',
	...
	foreach ( $wp_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			if ( ! is_dir( $theme_root . '/' . $dir ) || $dir[0] == '.' || $dir == 'CVS' )
				continue;
			if ( file_exists( $theme_root . '/' . $dir . '/dist/css/app.css' ) ) {
				// wp-content/themes/a-single-theme
				// wp-content/themes is $theme_root, a-single-theme is $dir
				$found_themes[ $dir ] = array(
					'theme_file' => $dir . '/dist/css/app.css',
				...
				foreach ( $sub_dirs as $sub_dir ) {
					if ( ! is_dir( $theme_root . '/' . $dir . '/' . $sub_dir ) || $dir[0] == '.' || $dir == 'CVS' )
						continue;
					if ( ! file_exists( $theme_root . '/' . $dir . '/' . $sub_dir . '/dist/css/app.css' ) )
						continue;
					$found_themes[ $dir . '/' . $sub_dir ] = array(
						'theme_file' => $dir . '/' . $sub_dir . '/dist/css/app.css',
						'theme_root' => $theme_root,
					);
					$found_theme = true;
				}
...
function validate_current_theme() {
	...
	if ( get_stylesheet() != WP_DEFAULT_THEME && !file_exists(get_template_directory() . '/dist/css/app.css') ) {
		switch_theme( WP_DEFAULT_THEME );
		return false;
	}

	if ( is_child_theme() && ! file_exists( get_stylesheet_directory() . '/dist/css/app.css' ) ) {
		switch_theme( WP_DEFAULT_THEME );
		return false;
	}
